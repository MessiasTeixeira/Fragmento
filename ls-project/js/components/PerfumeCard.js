...
// Criar fragâncias
export function criarCardFragancia(fragancia) {
           return `<div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="relative pb-2/3">
                <img src="${fragancia.imagem}" alt="${fragancia.nome}" class="absolute h-full w-full object-cover">
            </div>
            <div class="p-4">
                <h3 class="text-lg font-playfair font-bold text-gray-900">${fragancia.nome}</h3>
                <p class="mt-1 text-gray-500">${fragancia.descricao}</p>
                <div class="mt-2 flex items-center justify-between">
                    <span class="text-amber-800 font-bold">${formatarPreco(fragancia.preco)}</span>
                    <div class="flex gap-2">
                        <button id="adicionar-produto" class="btn-comprar bg-amber-800 text-white px-4 py-2 rounded-md hover:bg-amber-700 transition-colors">
                            Comprar
                        </button>
...
const containerFragancias = document.querySelector('#fragancias-container');

for (const fragancias of fragancia) {
     containerFragancias.insertAdjacentHTML('beforeend', criarCardFragancia(fragancias));
}
